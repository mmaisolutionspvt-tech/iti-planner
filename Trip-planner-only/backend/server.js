...
app.get('/api/config/maps', (req, res) => {
  res.json({
    googlePlacesKey: process.env.GOOGLE_PLACES_KEY || process.env.VITE_GOOGLE_PLACES_KEY || '',
    orsKey: process.env.ORS_KEY || process.env.VITE_ORS_KEY || ''
  });
});
...
async function executeFlightSearch({ from, to, date, adults = 1, children = 0, infants = 0, journeyType = '1' }) {
  try {
    ...
    if (fromIATA && toIATA) {
      ...
      const dsaRes = await axios.post(`${process.env.DSA_BASE_URL}/rest/Search`, {
        EndUserIp: process.env.DSA_END_USER_IP,
        ClientId: process.env.DSA_CLIENT_ID,
        UserName: process.env.DSA_USERNAME,
        Password: process.env.DSA_PASSWORD,
      ...
      }, { timeout: 12000, headers: { 'Api-Token': process.env.DSA_API_TOKEN } });
...
async function executeBusSearch({ from, to, date }) {
  try {
    ...
    if (fromCode && toCode) {
      const dsaRes = await axios.post(`${process.env.DSA_BUS_BASE_URL}/v9/rest/Search`, {
        ClientId: process.env.DSA_BUS_CLIENT_ID,
        UserName: process.env.DSA_BUS_USERNAME,
        Password: process.env.DSA_BUS_PASSWORD,
        FromCityCode: fromCode,
        ToCityCode: toCode,
        DepartDate: date,
      }, { timeout: 12000, headers: { 'Api-Token': process.env.DSA_BUS_API_TOKEN } });
...
app.post('/api/dsa/hotels/search', async (req, res) => {
  ...
  for (const cityId of cityIds) {
    ...
    try {
      const dsaRes = await axios.post(`${process.env.DSA_HOTEL_BASE_URL}/rest/Search`, {
        EndUserIp: process.env.DSA_HOTEL_END_USER_IP,
        ClientId: process.env.DSA_HOTEL_CLIENT_ID,
        UserName: process.env.DSA_HOTEL_USERNAME,
        Password: process.env.DSA_HOTEL_PASSWORD,
        CheckInDate: checkIn,
      ...
      }, { timeout: 12000, headers: { 'Api-Token': process.env.DSA_HOTEL_API_TOKEN } });
    ...
    for (const id of idsToTry) {
      ...
      try {
        const dsaRes = await axios.post(`${process.env.DSA_HOTEL_BASE_URL}/rest/Search`, {
          ...
          ReviewScore: null,
          IsNearBySearchAllowed: false,
        }, { timeout: 8000, headers: { 'Api-Token': process.env.DSA_HOTEL_API_TOKEN }, signal: opAbort.signal });

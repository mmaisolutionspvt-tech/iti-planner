...
export default function Step6Review({ 
  ...
  return (
    <div className="w-full p-4 sm:p-6">
      ...
      <RouteMapPanel 
        initialStops={wizardData.selectedPlaces || []} 
      ...
      <div className="bg-white p-5 rounded-3xl border border-gray-200 shadow-sm mb-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 text-xs">
          ...
          <div className="bg-gray-50 p-2.5 rounded-xl border border-gray-100">
            ...
            <span className="text-[9px] text-gray-400 block">({(wizardData.selectedRestaurants?.length || 0)} Slots)</span>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm">
          <div className="flex justify-between items-center mb-4 pb-3 border-b border-gray-100">
            <h3 className="font-bold text-gray-900 text-base flex items-center gap-2">
              ...
              Restaurant Reservations ({wizardData.selectedRestaurants.length})
            </h3>
            <button 
              onClick={() => onJumpToStep(5)}
              className="text-xs font-bold text-[#D4B15A] hover:underline flex items-center gap-1 cursor-pointer"
          ...
          {wizardData.selectedRestaurants.length === 0 ? (
            <p className="text-xs text-gray-400">No dining reservations added.</p>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">

              {wizardData.selectedRestaurants.map(r => (
                <div key={r.bookingId || r.id} className="bg-gray-50 p-3.5 rounded-2xl border border-gray-100 flex justify-between items-center">
                  <div>
                  ...
                  <span className="font-bold text-gray-800 text-sm">₹{(r.price || 0).toLocaleString()}</span>
                </div>
              ))}
            </div>
          )}
